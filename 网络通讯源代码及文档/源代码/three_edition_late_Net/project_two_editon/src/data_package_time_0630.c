...
int login_reply_func(char *news, char **outDataToUi)
{
	...
	{
		myprint("Error: The login_reply_func() dataLenth : %d is error", newslen);		
		assert(0);
	}
			...
			{				
				...
				else
				{
					...
					myprint( "Error: cmd : %d, package serial : %u checkCode : %u != *tmpCheckCode : %u",
									cmd , comHead->seriaNumber, checkCode, *tmpCheckCode);
					ret = -1;
				}		
			}
			else
...
{
	int ret = 0;
	char filePath[250] = { 0 }, fileName[50] = { 0 };			
	reqPackHead_t head;
	...
	memcpy(filePath, tmp + 1, strlen(tmp) - 2);			
	if((find_file_name(fileName, sizeof(fileName), filePath)) < 0)
	{
		myprint( "Error: func find_file_name() No find");
		assert(0);
	}
