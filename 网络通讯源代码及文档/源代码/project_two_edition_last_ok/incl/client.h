...
/**
 *Description: this function represents the dhcp reboot reboot
 *return Request state
 */
void *Reboot();

/**
 *Description: this function do the iniation job
 *return Init state
 */
void *Start();

/**
 *Description: this function represents the dhcp init state
 *return request state if no timeout occurs, if timeout occurs in this state start autoip policy
 */
void *Init();

/**
 *Description: this function represents the dhcp request state
 *@param xid: the dhcp xid field
 *@param *buildDhcpMsg: a function pointer used to build dhcp message
 *return bound state
 */
void *Request(unsigned xid,void (*buildDhcpMsg)(unsigned));

/**
 *Description: this function represents the dhcp bound state
 *return renew state if lease time out, if set infinite lease time, then just wait until a signal comes.
 */
void *Bound();

/**
 *Description: this function represents the dhcp renew state
 *return rebind state if T2 expires, return bound state if receive DHCPACK.
 */
void *Renew();

/**
 *Description: this function represents the dhcp rebind state
 *return bound state if receive DHCPACK and if timeout return stop state
 */
void *Rebind();
...
/**
 *Descritpion: stop the dhcp process
 *return init state.
 */
void *Stop();

/**
 *Description: send dhcp inform message
 *return stop state if find the ip address is in use, otherwise just return 0.
 */
void *Inform();

/**
 *Description: send dhcp decline message
 *return init state
 */
void *Decline();
